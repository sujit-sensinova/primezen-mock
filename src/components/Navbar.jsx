        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <Magnetic strength={0.2}>
            <a href="#" className="relative z-50 flex items-center gap-2 group outline-none focus-visible:ring-2 focus-visible:ring-brand rounded-sm">
              <img
                src={isScrolled ? "/logo/SVG/logo-dark.svg" : "/logo/SVG/logo-light.svg"}
                alt="PrimeZen Logo"
          ...
          <nav className="hidden md:flex items-center gap-10">
            {navLinks.map((link) => (
              <Magnetic key={link.name} strength={0.4}>
                <a
                  href={link.href}
                  className={`text-sm font-semibold tracking-wide transition-colors duration-300 outline-none focus-visible:ring-2 focus-visible:ring-brand rounded-sm px-2 py-1 relative group ${isScrolled ? 'text-text-secondary hover:text-black' : 'text-white/80 hover:text-white'
                    }`}
                >
                  {link.name}
                  {/* Hover underline */}
          ...
          {/* Mobile Menu Toggle */}
          <button
            className={`md:hidden relative z-50 p-2 outline-none focus-visible:ring-2 focus-visible:ring-brand rounded-sm transition-colors ${isScrolled || mobileMenuOpen ? 'text-black' : 'text-white'}`}
